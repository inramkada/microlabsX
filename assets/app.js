...
const NUM_PARTICLES = REDUCED_MOTION ? 150 : (MOBILE_MODE ? 350 : Math.min(1000, CORES * 250));
const MAX_CONNECTIONS = REDUCED_MOTION ? 2 : (MOBILE_MODE ? 4 : 8);
const SPEED = REDUCED_MOTION ? 0.35 : 0.6;
const X_ZONE_MARGIN = 45;
const X_SAT_THRESHOLD = MOBILE_MODE ? 35 : 85;
...
function inXZone(x, y) {
    ...
    return (d1 < X_ZONE_MARGIN || d2 < X_ZONE_MARGIN);
}
...
function createParticle() {
    ...
    return {
        ...
        radius: 1.5, inX: false, prevInX: false,
        hexCode: randomHex()
    };
}
...
window.addEventListener("resize", () => {
    ...
    particles.forEach(p => {
        p.ox = (p.ox / oldW) * width;
        p.oy = (p.oy / oldH) * height;
    });
});
    ...
    for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length && connections < MAX_CONNECTIONS; j++) {
            ...
            if (dist2 < MAX_DIST2) {
                ...
                ctx.strokeStyle = `rgba(0, 255, 102, ${(1 - dist / MAX_DIST) * 0.5})`;
                ctx.beginPath();
...
function drawDeathMarkers(now) {
    ...
    for (let i = deathMarkers.length - 1; i >= 0; i--) {
        ...
        ctx.save();
        ctx.strokeStyle = `rgba(255, 255, 255, ${(1 - t) * 0.4})`;
        ctx.lineWidth = 0.8;
        for (let k = 0; k < 4; k++) {
            const a = m.angle + (Math.PI * 2 * k / 4);
            const len = 2 + (8 * t);
            ctx.beginPath();
            ctx.moveTo(m.x, m.y);
